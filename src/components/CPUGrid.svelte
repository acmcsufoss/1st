<script lang="ts">
  ...
  function showHover(i: number, j: number) {
    const pin = pins.find(p => p.location[0] === i && p.location[1] === j);
    if (pin) {
      hovered = {
        name: pin.contributorName,
        x: i,
        y: j
      };
    }
  ...
  function isTaken(i: number, j: number) {
    return pins.some(p => p.location[0] === i && p.location[1] === j);
  }

  function getColor(i: number, j: number) {
    return pins.find(p => p.location[0] === i && p.location[1] === j)?.color || pinDefaultColor;
  }

  function getPin(i: number, j: number) {
    return pins.find(p => p.location[0] === i && p.location[1] === j);
  }
...
<svg width="600" height="600" viewBox="0 0 600 536" fill="none" xmlns="http://www.w3.org/2000/svg" style="margin:auto;">
      ...
      {#if !skippedGrid.has(`${i},${j}`)}
        {#if isTaken(i, j)}
          <circle
            ...
            on:click={() => {
              openContributorDialog(pins.indexOf(getPin(i, j)));
            }}
          />
        {:else}
  ...
  {/if}
  <svg width="242" height="242" x="153" y="147" viewBox="0 0 242 242" fill="none" xmlns="http://www.w3.org/2000/svg">
