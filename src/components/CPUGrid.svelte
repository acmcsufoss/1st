<script lang="ts">
  ...
  let contributorLocation = { x: -1, y: -1 }
  let hoveredContributor = "";

  function showHover(i, j) {
    console.log("showhover")
    var boxID = i+"_"+j+"_hover";
    var textID = i+"_"+j+"_text";
    var name = getContNameFromLocation([i,j]);
    var boxWidth = (name.length*12);
    var boxX = i*42-((name.length-6)/2);

    hoveredContributor = name;
    contributorLocation = { x: i, y: j }

    document.getElementById(boxID).style.opacity = ".8";
    document.getElementById(textID).innerHTML = name;
    document.getElementById(boxID).setAttribute("width", boxWidth.toString());
    document.getElementById(boxID).setAttribute("x", boxX.toString());
    document.getElementById(textID).style.opacity = "1";
  }
  function hideHover(i, j) {
    var boxID = i+"_"+j+"_hover";
    var textID = i+"_"+j+"_text";
    document.getElementById(boxID).style.opacity = "0";
    document.getElementById(textID).style.opacity = "0";
  }
  function getContNameFromLocation(loc) {
    for (let i = 0; i < pins.length; i++) {
      if (pins[i].location[0] === loc[0] && pins[i].location[1] === loc[1]) {
        return pins[i].contributorName;
      }
    }
    return "";
  }
        ...
        {#each {length: 12} as _, j}
            {#if !isIndexInList([i,j], skippedGrid)}
                {#if isIndexInList([i,j], takenGrid)}  
                    <circle id={i+"_"+j} 
                      onmouseover={() => showHover(i, j)} 
                      onmouseout={() => hideHover(i, j)}
                      cx={i*42+48} cy={j*42+36} r="12" fill={pinColors[Math.floor(Math.random() * pinColors.length)]} />
                {:else}
                    <circle cx={i*42+48} cy={j*42+36} r="12" fill={pinDefaultColor} />
                {/if}
            {/if}
        {/each}
    {/each}
    <rect id={contributorLocation.x +"_" + contributorLocation.y +"_hover"} 
      x={contributorLocation.x*42+6} y={contributorLocation.y*42-6}
      height="28"
      fill="#2b2b2b" style="opacity: 0; z-index: 100000;" />
    <text id={contributorLocation.x+"_"+contributorLocation.y+"_text"} 
      font-size="16" fill="#FFFFFE" 
      x={contributorLocation.x*42+4} y={contributorLocation.y*42+12} style="opacity: 0;" >
        {hoveredContributor}
    </text>


    <svg width="242" height="242" viewBox="0 0 242 242" x="159" y="147" fill="none" xmlns="http://www.w3.org/2000/svg">
